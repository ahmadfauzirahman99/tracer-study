<div class="panel panel-default">
    ...
    <div class="panel-body">
            ...
            <div class="">
                ...
                <form class="" action="<?= $this->url->get('hasil_kuisioner/add') ?>" method="post">
                    <div class="alert alert-info">
                        ...
                        <h4 for="">Username Alumni : <?= $this->session->get('ses_username') ?></h4>
                            <input required type="hidden" class="form-control" id="" placeholder="Username penjawab kuisioner alumni..." name="username" value="<?= $this->session->get('ses_username') ?>">
                            <input required type="hidden" class="form-control" value="alumni" placeholder="Nama penjawab kuisioner..." name="status">
                        </div>
                    </div>
                    <div class="panel panel-default">
                      <div class="panel-heading">
                        <h4>Form Nama Perusahaan</h4>
                      </div>
                      <div class="panel-body">
                          <div class="form-group">
                            <label for="">Nama Perusahaan Sekarang</label>
                            <select class="form-control" name="perusahaan_sekarang" >
                                <?php foreach ($perusahaans as $perusahaan): ?>
                                    <option  value="<?= $perusahaan->id_perusahaan ?>"><?= $perusahaan->nama ?></option>
                                <?php endforeach; ?>
                            </select>
                          </div>
                          <div class="form-group">
                            <label for="">Nama Perusahaan Pertama</label>
                            <select class="form-control" name="perusahaan_pertama" >
                                <?php foreach ($perusahaans as $perusahaan): ?>
                                    <option  value="<?= $perusahaan->id_perusahaan ?>"><?= $perusahaan->nama ?></option>
                                <?php endforeach; ?>
                            </select>
        ...
        <?php else: ?>
            ...
            <a type="button" href="<?= $this->url->get('dashboard/hasil/alumni') ?>" class="btn btn-info">
